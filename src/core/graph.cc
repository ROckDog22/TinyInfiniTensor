...
namespace infini
{
            ...
            while(current.lock()->getTargets().size()){
                if(current.lock()->getTargets()[0]->getOpType() == OpType::Transpose  && 
                    current.lock()->getTargets()[0]->getOutputs()[0]->getTargets().size()){
                        WRef<OperatorObj> next_op = refs_to_wrefs<OperatorObj>({current.lock()->getTargets()[0]->getOutputs()[0]->getTargets()[0]})[0];
    ...
    void GraphObj::dataMalloc()
    {
        ...
        std::unordered_map<TensorObj*, size_t> dic;
        for(auto& tensor: tensors){
            dic[tensor.get()] = allocator.alloc(tensor->getBytes());
        }
        for(auto& tensor: tensors){
            auto ptr = static_cast<uint8_t*>(allocator.getPtr()) + dic[tensor.get()];
